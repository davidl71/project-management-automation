...
def analyze_task_clarity(
    ...
    try:
        ...
        for task in pending:
            ...
            # 2. Check if estimate is too large (>4 hours)
        ...
        logger.error(f"Failed to analyze task clarity: {e}", exc_info=True)
        return {
            "status": "error",
            "error": str(e)
        }
...
def _apply_improvements_file_based(analysis_result: Dict, output_path: Optional[str]) -> Dict[str, Any]:
    """Fallback: Apply improvements by writing to file directly."""
    project_root = find_project_root()
    state_file = project_root / ".todo2" / "state.todo2.json"
