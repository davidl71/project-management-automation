...
def reload_module(module_name: str) -> Dict[str, Any]:
    ...
    try:
        if module_name not in sys.modules:
            return {"module": module_name, "status": "skipped", "reason": "not loaded"}

        module = sys.modules[module_name]
        importlib.reload(module)

        return {"module": module_name, "status": "reloaded", "file": getattr(module, "__file__", None)}
...
def reload_all_modules(package_name: str = "project_management_automation") -> Dict[str, Any]:
    ...
    if not modules:
        return {"success": False, "error": f"No modules found for package: {package_name}"}
    ...
    return {
        ...
        "summary": {"total_modules": len(modules), "reloaded": reloaded, "errors": errors, "skipped": skipped},
        "modules": results if errors > 0 else None,  # Only show details if errors
        "version": version_refreshed,
        "message": f"Reloaded {reloaded} modules in {duration:.2f}s" + (f" ({errors} errors)" if errors else ""),
    }
...
def reload_specific_modules(module_names: List[str]) -> Dict[str, Any]:
    ...
    """
    if not is_dev_mode():
        return {"success": False, "error": "Dev mode not enabled. Set EXARP_DEV_MODE=1 in environment."}
    ...
    return {
        "success": errors == 0,
        "summary": {"requested": len(module_names), "reloaded": reloaded, "errors": errors},
        "modules": results,
    }
    ...
    for name in sorted(modules):
        ...
        if module:
            ...
            if file_path and Path(file_path).exists():
                mtime = datetime.fromtimestamp(Path(file_path).stat().st_mtime).isoformat()

            module_info.append({"name": name, "file": file_path, "modified": mtime})

    return {"dev_mode": is_dev_mode(), "total_modules": len(modules), "modules": module_info}
